...
export const appRouter = router({
  ...
  audit: router({
    submit: publicProcedure.input(z.object({
      ...
      try {
        ...
        const submission = await saveFormSubmission({ formName: "Free Funnel Audit", contactId: contact.contactId, data: input, attribution: input.attribution });
  ...
  crm: router({
    ...
    directory: adminProcedure.input(z.object({ search: z.string().max(160).optional(), stage: z.string().max(80).optional(), niche: z.string().max(180).optional(), source: z.string().max(180).optional(), minScore: z.number().int().min(0).max(100).optional(), maxScore: z.number().int().min(0).max(100).optional(), dateFrom: z.string().datetime().optional(), dateTo: z.string().datetime().optional(), booked: z.boolean().optional(), page: z.number().int().min(1).optional(), pageSize: z.number().int().min(1).max(100).optional() }).optional()).query(({ input }) => getCrmLeadDirectory(input || {})),
